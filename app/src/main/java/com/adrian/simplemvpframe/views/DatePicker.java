    ...
    public void onClick(View v) {
        ...
        if (i == R.id.tv_confirm) {
            if (mOnDateCListener != null) {
                int currentItem = mDayWheelView.getSelectedItem();
                currentItem = currentItem >= days.size() - 1 ? days.size() - 1 : currentItem;
                mOnDateCListener.onDateSelected(years.get(mYearWheelView.getSelectedItem()), months.get(mMonthWheelView.getSelectedItem()), days.get(currentItem));
            }
        }
        cancel();
    }
