 ...
 * description：
 */
class FantasticButton : LinearLayout {
    ...
    //格式化进度值为特殊格式
    var mProgressFormatter: CircleProgressBar.ProgressFormatter = CircleProgressBar.DefaultProgressFormatter()
        set(value) {
            field = value
            invalidate()
        }
    ...
    constructor(context: Context?, attrs: AttributeSet?, defStyleAttr: Int) : super(context, attrs, defStyleAttr) {
        ...
        val a = context.obtainStyledAttributes(attrs, R.styleable.FantasticButton, defStyleAttr, 0)

        mLineCount = a.getInt(R.styleable.FantasticButton_line_count, CircleProgressBar.DEFAULT_LINE_COUNT)
        mStopAnimType = a.getInt(R.styleable.FantasticButton_stop_anim_type, CircleProgressBar.STOP_ANIM_SIMPLE)
        mStyle = a.getInt(R.styleable.FantasticButton_style, CircleProgressBar.LINE)
        mShader = a.getInt(R.styleable.FantasticButton_progress_shader, CircleProgressBar.LINEAR)
        mCap = if (a.hasValue(R.styleable.FantasticButton_progress_stroke_cap)) Paint.Cap.values()[a.getInt(R.styleable.FantasticButton_progress_stroke_cap, 0)] else Paint.Cap.BUTT
        mLineWidth = a.getDimension(R.styleable.FantasticButton_line_width, Utils.dip2px(context, CircleProgressBar.DEFAULT_LINE_WIDTH))
        mProgressTextSize = a.getDimension(R.styleable.FantasticButton_progress_text_size, CircleProgressBar.DEFAULT_PROGRESS_TEXT_SIZE)
        mProgressStrokeWidth = a.getDimension(R.styleable.FantasticButton_progress_stroke_width, CircleProgressBar.DEFAULT_PROGRESS_STROKE_WIDTH)
        mProgressStartColor = a.getColor(R.styleable.FantasticButton_progress_start_color, Color.parseColor(CircleProgressBar.COLOR_FFF2A670))
        mProgressEndColor = a.getColor(R.styleable.FantasticButton_progress_end_color, Color.parseColor(CircleProgressBar.COLOR_FFF2A670))
        mProgressTextColor = a.getColor(R.styleable.FantasticButton_progress_text_color, Color.parseColor(CircleProgressBar.COLOR_FFF2A670))
        mProgressBackgroundColor = a.getColor(R.styleable.FantasticButton_progress_background_color, Color.parseColor(CircleProgressBar.COLOR_FFD3D3D5))
        mStartDegree = a.getFloat(R.styleable.FantasticButton_progress_start_degree, CircleProgressBar.DEFAULT_START_DEGREE)
        mDrawBackgroundOutsideProgress = a.getBoolean(R.styleable.FantasticButton_drawBackgroundOutsideProgress, false)
        mCenterColor = a.getColor(R.styleable.FantasticButton_center_color, Color.TRANSPARENT)
        mShowValue = a.getBoolean(R.styleable.FantasticButton_show_value, true)
        mCenterDrawable = a.getDrawable(R.styleable.FantasticButton_center_src)
    ...
    private fun initPaint() {
        ...
        mProgressPaint.style = if (mStyle == CircleProgressBar.SOLID) Paint.Style.FILL else Paint.Style.STROKE
        mProgressPaint.strokeWidth = mProgressStrokeWidth
        mProgressPaint.color = mProgressStartColor
        mProgressPaint.strokeCap = mCap

        mProgressBackgroundPaint.style = if (mStyle == CircleProgressBar.SOLID) Paint.Style.FILL else Paint.Style.STROKE
        ...
        if (mProgressStartColor != mProgressEndColor) {
            ...
            when (mShader) {
                CircleProgressBar.LINEAR -> { //线性渐变
                    ...
                    val matrix = Matrix()
                    matrix.setRotate(CircleProgressBar.LINEAR_START_DEGREE, mCenterX, mCenterY)
                    shader.getLocalMatrix(matrix)
                }
                CircleProgressBar.RADIAL -> { //径向渐变
                    if (mRadius <= 0) return
                    shader = RadialGradient(mCenterX, mCenterY, mRadius, mProgressStartColor, mProgressEndColor, Shader.TileMode.CLAMP)
                }
                CircleProgressBar.SWEEP -> {  //扫描渐变
                    if (mRadius <= 0) return
                    val radian = mProgressStrokeWidth / Math.PI * 2f / mRadius
                    val rotateDegrees: Float = -(if (mCap == Paint.Cap.BUTT && mStyle == CircleProgressBar.SOLID_LINE) 0f else Math.toDegrees(radian).toFloat())
    ...
    private fun drawCenterColor(canvas: Canvas?) {
        if (mStyle == CircleProgressBar.LINE || mStyle == CircleProgressBar.SOLID_LINE) {
    ...
    private fun drawCenterDrawable(canvas: Canvas?) {
        try {
            if ((mStyle == CircleProgressBar.LINE || mStyle == CircleProgressBar.SOLID_LINE) && mCenterDrawable != null) {
    ...
    private fun drawProgress(canvas: Canvas?) {
        when (mStyle) {
            CircleProgressBar.SOLID -> drawSolidProgress(canvas)
            CircleProgressBar.SOLID_LINE -> drawSolidLineProgress(canvas)
    ...
    private fun drawSolidProgress(canvas: Canvas?) {
        if (mDrawBackgroundOutsideProgress) {
            val startAngle: Float = CircleProgressBar.MAX_DEGREE * mProgress / mMax
            val sweepAngle: Float = CircleProgressBar.MAX_DEGREE - startAngle
            canvas?.drawArc(mProgressRectF, startAngle, sweepAngle, true, mProgressBackgroundPaint)
        } else {
            canvas?.drawArc(mProgressRectF, 0f, CircleProgressBar.MAX_DEGREE, true, mProgressBackgroundPaint)
        }
        canvas?.drawArc(mProgressRectF, 0f, CircleProgressBar.MAX_DEGREE * mProgress / mMax, true, mProgressPaint)
    }
    ...
    private fun drawSolidLineProgress(canvas: Canvas?) {
        if (mDrawBackgroundOutsideProgress) {
            ...
            canvas?.drawArc(mProgressRectF, 0f, CircleProgressBar.MAX_DEGREE, false, mProgressBackgroundPaint)
        }
        canvas?.drawArc(mProgressRectF, 0f, CircleProgressBar.MAX_DEGREE * mProgress / mMax, false, mProgressPaint)
    }
    ...
    fun stopAnimator() {
        if (mAnimator != null && mAnimator!!.isRunning) {
            ...
            if (mStopAnimType == CircleProgressBar.STOP_ANIM_SIMPLE) {    //直接停止动画并恢复到进度0
                mAnimator?.cancel()
                mProgress = 0
            } else {    //动画回退到进度0
                mAnimator?.reverse()
